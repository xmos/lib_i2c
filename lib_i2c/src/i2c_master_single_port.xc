...
static void wait_for_clock_high(
  port p_i2c,
  static const unsigned SCL_HIGH,
  unsigned &fall_time,
  unsigned delay)
{
...
static void high_pulse_drive(
  ...
  static const unsigned kbits_per_second,
  static const unsigned SCL_HIGH,
  static const unsigned SDA_HIGH,
  static const unsigned S_REST,
  unsigned &fall_time)
{
  const unsigned bit_time = BIT_TIME(kbits_per_second);
  ...
  unsigned &fall_time)
{
  ...
  for (int i = 8; i != 0; i--) {
    high_pulse_drive(p_i2c, bit_rev_data & 1, kbits_per_second, SCL_HIGH, SDA_HIGH, S_REST, fall_time);
    bit_rev_data >>= 1;
  }
  return high_pulse_sample(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, S_REST, fall_time);
}
  ...
  static const unsigned other_bits_mask)
{
  ...
  while (1) {
    ...
    case (size_t i =0; i < n; i++)
      ...
      start_bit(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time, stopped);
      int ack = tx8(p_i2c, (device << 1) | 1, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
      if (ack == 0) {
        for (int j = 0; j < m; j++){
          unsigned char data = 0;
          timer tmr;
          for (int i = 8; i != 0; i--) {
            int temp = high_pulse_sample(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
            data = (data << 1) | temp;
          }
          buf[j] = data;
      ...
      if (send_stop_bit) {
        stop_bit(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
        locked_client = -1;
      } else {
        locked_client = i;
      }
    ...
    case (size_t i = 0; i < n; i++)
      ...
      unsigned fall_time = last_fall_time;
      start_bit(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time, stopped);
      int ack = tx8(p_i2c, device<<1, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
      int j = 0;
      for (; j < n; j++) {
        if (ack != 0)
          break;

        ack = tx8(p_i2c, buf[j], kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
      }
      if (send_stop_bit) {
        stop_bit(p_i2c, kbits_per_second, SCL_HIGH, SDA_HIGH, other_bits_mask, fall_time);
        locked_client = -1;
